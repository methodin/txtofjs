...
var Txtof = {
    ...
    colIndex: 1,
    init: function() {
        this.page = new Txtof.Page("default");
    ...
    parse: function(content) {
        ...
        for (var i in lines) {
            Txtof.page.addRow(new Txtof.Row());
            if (lines[i].trim() == "") {
                continue;
            }
            Txtof.working = null;
                ...
                switch (c) {
                    ...
                    case '-': 
                        Txtof.page.getCurrentRow().getCol(Txtof.colIndex-1)
                            .appendStr(new Txtof.Working('\0', TYPE_HR).compile());
                        return;
                    case '!': 
                        Txtof.working = new Txtof.Working('\n', TYPE_HEADING);
                        continue;
                    case '|':
                        Txtof.mode = 'form';
                        continue;
                    case '#':
                        Txtof.pages.push(Txtof.page);
                        Txtof.page = new Txtof.Page(line.substring(1));
                        return;
                } 
            }
                ...
                switch (true) {
                    ...
                    case c == '|':
                        ...
                        if (row.colCount() < Txtof.colIndex + 1) {
                            row.addCol(new Txtof.Col());
                        }
                        Txtof.colIndex++;
                        row.getCol(Txtof.colIndex-1).addBuf(); 
                        continue ;
                    case c == '[':
                        Txtof.working = new Txtof.Working(']', TYPE_UNKNOWN);
                        continue;
                    case c == '{':
                        Txtof.working = new Txtof.Working('}', TYPE_LABEL);
                        continue;
                    case c == '(':
                        Txtof.working = new Txtof.Working(')', TYPE_BUTTON, TYPE_BUTTON_UNKNOWN);
                        continue;
                    case c == '<':
                        Txtof.working = new Txtof.Working('>', TYPE_SELECT);
                        continue;
                    case c == '%':
                        Txtof.working = new Txtof.Working(' ', TYPE_BIND);
                        continue;
    ...
    Working: function(char, type, secondaryType) {
        this.str = '';
        this.until = char;
        this.type = type;
